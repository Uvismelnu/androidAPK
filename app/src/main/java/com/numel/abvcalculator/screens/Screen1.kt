...
@Composable
fun Screen1(navController: NavController, screenData: ScreenData, viewModel: MainViewModel) {
    var newText by remember { mutableStateOf(screenData.text) }

    Column(modifier = Modifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally) {
        TextField(value = newText, onValueChange = { newText = it }, label = { Text("Editar texto") })
        Button(onClick = {
